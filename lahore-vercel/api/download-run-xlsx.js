...
function sanitizeFilename(name) {
  return String(name || "lahore-output.xlsx").replace(/[^A-Za-z0-9._-]/g, "_");
}
...
async function resolveArtifactId(cfg, runId) {
  ...
  const artifact =
    artifacts.find((item) => item.name === "scraper-xlsx-lahore") ||
    artifacts.find((item) => item.name === "scraper-output-lahore");
